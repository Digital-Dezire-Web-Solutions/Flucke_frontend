...
export default function DashboardTab({ user, onEditPersonalInfo, onViewAllOrders, onEditAddress, onGoToTab }) {
                ...
                {user.orders.slice(0, 3).map((order) => (
                    <div className="rl-order-row" key={order._id}>
                        ...
                        <div className="rl-order-row__right">
                            ...
                            <span className="rl-order-row__total">₹{order.total.toFixed(2)}</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            {defaultAddress && (
                <div className="rl-card">
                    <div className="rl-card__head">
                        ...
                        <button className="rl-link-btn" onClick={() => onEditAddress?.(defaultAddress)}>
                            Edit
                        </button>
                    </div>
